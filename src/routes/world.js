...
router.get('/info', async (req, res) => {
  try {
    ...
    const worldInfo = {
      ...
      era: decadeString,
      eraMultiplier: require('../services/eraEconomicService').getEraMultiplier(currentYear),
      status: world.status,
      ...
      logoSvg: membership?.logoSvg || null,
      backgroundColor: membership?.backgroundColor || null,
      primaryColor: membership?.primaryColor || null,
      secondaryColor: membership?.secondaryColor || null,
...
router.post('/acceleration', async (req, res) => {
  try {
    ...
    if (!factor || factor <= 0 || factor > 120) {
      return res.status(400).json({ error: 'Invalid acceleration factor (max 120x)' });
    }
